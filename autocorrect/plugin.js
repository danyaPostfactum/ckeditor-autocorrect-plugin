...
(function () {
  ...
  function CharacterIterator(range) {
    ...
    walker.evaluator = function (node) {
      return (
        (node.type === CKEDITOR.NODE_TEXT && !isBookmark(node)) ||
  ...
  CharacterIterator.prototype.previousCharacter = function () {
    while (this.referenceCharacterOffset === 0) {
      ...
      if (!this.referenceNode) return null;
      if (
        this.referenceNode.type === CKEDITOR.NODE_ELEMENT &&
        this.referenceNode.getName() == 'br'
      )
        return null;
      this.referenceCharacterOffset = this.referenceNode.getText().length;
    }
    if (this.referenceCharacterOffset === 0) return null;
    this.referenceCharacter =
      this.referenceNode.getText()[--this.referenceCharacterOffset];
    ...
    init: function (editor) {
      ...
      editor.addCommand('autocorrect', {
        exec: function (editor) {
          ...
          walker.evaluator = function (node) {
            return node.type === CKEDITOR.NODE_TEXT && !isBookmark(node);
          };
          var node;
          while ((node = walker.next())) {
            var next = getNext(node);
            var parent = getBlockParent(node);
            walker.current = correctTextNode(
              node,
              (parent.isBlockBoundary() && !next) ||
                (next &&
      ...
      var command = editor.addCommand('toggleAutocorrect', {
        ...
        exec: function (editor) {
          this.setState(
            isEnabled() ? CKEDITOR.TRISTATE_OFF : CKEDITOR.TRISTATE_ON
          );
        },
      });
      ...
      var optionsCommand = editor.addCommand(
        'autocorrectOptions',
      ...
      editor.ui.add('AutoCorrect', CKEDITOR.UI_MENUBUTTON, {
        ...
        onMenu: function () {
          editor.getMenuItem('autoCorrectWhileTyping').label = isEnabled()
            ? lang.disable
            : lang.enable;
      ...
      function correctOnInput(cursor) {
        ...
        var input = cursor.startContainer
          .getText()
          .substring(cursor.startOffset - 1, cursor.startOffset);
        ...
        if (isWhitespace(input)) autoCorrectOnWhitespace(cursor, input);
      ...
      function correctOnEnter(cursor) {
        ...
        if (getOption('replaceHyphens')) replaceHyphens(cursor, '');
      ...
      function correctTextNode(node, isBlockEnding) {
        ...
        while (cursor.startOffset < cursor.startContainer.getText().length) {
          ...
          if (
            isBlockEnding &&
            cursor.endOffset == cursor.startContainer.getText().length
          ) {
            // "Emulate" Enter key
      ...
      function replaceRangeContent(range, data) {
        var walker = new CKEDITOR.dom.walker(range);
        walker.evaluator = function (node) {
          return node.type === CKEDITOR.NODE_ELEMENT && isBookmark(node);
        };
        var bm;
        var bookmarks = [];
      ...
      function replaceHyphenPairInWord(cursor, input) {
        ...
        if (input && !iterator.previousCharacter()) return;
        ...
        while ((ch = iterator.previousCharacter()) && !isWhitespace(ch)) {
          if (ch == needle[i]) {
            if (i == needle.length - 1)
              hypenPairRange.setEnd(
                iterator.referenceNode,
                iterator.referenceCharacterOffset + 1
              );
            i--;
            if (i < 0) break;
          } else {
        ...
        if (
          charAfterHypens &&
          !(
            isWhitespace(charAfterHypens) ||
            isWordPart(charAfterHypens) ||
            isPairOpenCharacter(charAfterHypens)
          )
        )
          return false;

        hypenPairRange.setStart(
          iterator.referenceNode,
          iterator.referenceCharacterOffset
        );
        ...
        if (
          !charBeforeHypens ||
          !(
            isWordPart(charBeforeHypens) ||
            isPairClosingCharacter(charBeforeHypens)
          )
        )
          return false;
      ...
      function replaceHyphens(cursor, input) {
        ...
        if (!isWhitespace(charBeforeHyphen)) return false;
      ...
      function autoCorrectOnWhitespace(cursor, inputChar) {
        if (
          getOption('formatBulletedLists') &&
          formatBulletedList(cursor, inputChar)
        )
          return;

        if (
          getOption('formatNumberedLists') &&
          formatNumberedList(cursor, inputChar)
        )
          return;

        if (getOption('replaceHyphens')) replaceHyphens(cursor, inputChar);
      }
      ...
      function autoCorrectOnDelimiter(cursor, delimiter) {
        ...
        if (getOption('formatOrdinals')) formatOrdinals(cursor, delimiter);
      ...
      function formatHyperlink(cursor, input) {
        ...
        beforeReplace();
        var link = editor
          .elementPath(matchRange.getCommonAncestor())
          .contains('a', 1);
      ...
      function formatOrdinals(cursor, delimiter) {
        ...
        if (delimiter && !iterator.previousCharacter()) return;
        ...
        if (!(suffix in suffixes)) return false;
        ...
        while ((ch = iterator.previousCharacter())) {
          if (ch >= '0' && ch <= '9') number = ch + number;
          else if (isWordPart(ch)) return false;
          else break;
        }

        var n = number % 100;
        if (n > 9 && n < 20) {
          if (suffix !== 'th') return false;
        } else {
          n = number % 10;
          if (n == 1) {
            if (suffix !== 'st') return false;
          } else if (n == 2) {
            if (suffix !== 'nd') return false;
          } else if (n == 3) {
            if (suffix !== 'rd') return false;
          } else if (suffix !== 'th') return false;
        }
      ...
      function formatBulletedList(cursor, input) {
        ...
        if (!marker || !(marker in bulletedListMarkers)) return false;

        markerRange.setStart(
          iterator.referenceNode,
          iterator.referenceCharacterOffset
        );

        if (iterator.previousCharacter()) return false;
      ...
      function formatNumberedList(cursor, input) {
        ...
        if (!(delimiter in { '.': true, ')': true })) return false;
        ...
        var type;
        if (start.match(/^[0-9]+$/)) type = '1';
        else if (start.match(/^[ivxlcdm]+$/)) type = 'i';
        else if (start.match(/^[IVXLCDM]+$/)) type = 'I';
        else if (start.match(/^[a-z]$/)) type = 'a';
        else if (start.match(/^[A-Z]$/)) type = 'A';
        else return false;
        ...
        if (
          ...
          previous.type == CKEDITOR.NODE_ELEMENT &&
          previous.getName() == 'ol' &&
          previous.getAttribute('type') == type &&
          getLastNumber(previous) == startNumber - 1
        ) {
          appendContentsToList(parent, previous);
        } else {
          var attributes =
      ...
      function replaceContentsWithList(listContents, type, attributes) {
        ...
        if (attributes) listNode.setAttributes(attributes);
      ...
      editor.on('key', function (event) {
        if (event.data.keyCode != 2228237 && event.data.keyCode != 13) return;
        if (!isEnabled()) return;
      ...
      editor.on('contentDom', function () {
        editor.editable().on('keypress', function (event) {
          if (event.data.$.charCode === 0) return;
          if (!isEnabled()) return;
